...
engine beeing completely different and also much faster."
  ...
  (setq helm-source-occur
        (car (helm-occur-pydef-build-sources (list (current-buffer)) "Helm occur")))
...
(defun helm-occur-pydef-format-disp (linum type symbol)
  (let ((typ (if (equal typ "=") "variable" typ)))
    (format "%s: %s (%s)"
                             (propertize
                             linum
                             'help-echo (buffer-file-name
                                           (get-buffer buf)))
                             (propertize sym 'face 'helm-grep-lineno) typ)
  ))
(defun helm-occur-pydef-format-search (linum type symbol)
  (let ((typ (if (equal typ "=") "variable" typ)))
    (format "%s (%s)" (propertize sym 'face 'helm-grep-lineno) typ)
  ))

(defun helm-occur-pydef-filter-transformer (candidates source)
  "Return CANDIDATES prefixed with line number."
  (cl-loop with buf = (helm-get-attr 'buffer-name source)
           for c in candidates collect
           (when (string-match helm-occur-pydef-candidate-pattern c)
             (let ((sym (match-string 1 c))
                   (typ (match-string 2 c))
                   (linum (match-string 3 c)))
               (cons (helm-occur-pydef-format-disp linum type symbol)
...
(setq helm-occur-pydef-candidate-search
  "\\`\\(?3:[0-9]*\\) \\(?:\\(?2:def\\|class\\) *\\(?1:%s\\)(\\|\\(?1:%s\\) *\\(?2:=\\)\\).*")
(setq helm-occur-pydef-candidate-pattern (format helm-occur-pydef-candidate-search "[[:alnum:]_]*" "[[:alnum:]_]*"))

(defun helm-occur-filter-pydefs-transformer (candidates)
  (seq-filter (lambda (candidate) 
                (string-match-p helm-occur-pydef-candidate-pattern candidate)) candidates))

(defun helm-occur-pydef-build-sources (buffers &optional source-name)
  ...
  (cl-loop for buf in buffers
           ...
           (helm-make-source (or source-name bname)
             ...
             (lambda (candidate)
               ...
               ;; and not "12:foo".
               (when (string-match helm-occur-pydef-candidate-pattern
                                   candidate)
                 (helm-occur-pydef-format-search nil (match-string 2 candidate) (match-string 1 candidate))))
             :search (lambda (pattern)
                       (when (string-match "\\`\\^\\([^ ]*\\)" pattern)
                         (setq pattern (let ((symbol (match-string 1 pattern))) (format helm-occur-pydef-candidate-search symbol))))
                       (condition-case _err
             ...
             :filtered-candidate-transformer 'helm-occur-pydef-filter-transformer
             :candidate-transformer 'helm-occur-filter-pydefs-transformer
